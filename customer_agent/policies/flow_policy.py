...
from customer_agent.dialogue_understanding.flow import FlowExecutor, FlowsList
from customer_agent.dialogue_understanding.stack.stack_frame import FlowStackFrame
from customer_agent.shared.constants import ACTION_LISTEN
...
class FlowPolicy(Policy):
        ...
        try:
            ...
            for _ in range(max_iterations):
                ...
                if result.action:
                    ...
                    # 构建 metadata
                    prediction_metadata = self._build_action_metadata(
                        tracker, result.next_step_id, is_final_action
                    )

                    # 如果是嵌套 collect 步骤，设置 slot_to_collect
                    if result.metadata.get("slot_to_collect"):
                        slot_to_collect = result.metadata["slot_to_collect"]
                        prediction_metadata["slot_to_collect"] = slot_to_collect
                        if result.metadata.get("fallback_action"):
                            prediction_metadata["fallback_action"] = result.metadata["fallback_action"]
                        # 更新 FlowStackFrame 的 slot_to_collect
                        flow_frame = tracker.dialogue_stack.top_flow_frame()
                        if flow_frame:
                            flow_frame.slot_to_collect = slot_to_collect
                        logger.debug(f"Nested collect step: slot_to_collect={slot_to_collect}")
    ...
    def _preset_slot_to_collect_if_needed(
        ...
        # 检查下一步是否是 collect 步骤
        from customer_agent.dialogue_understanding.flow.flow import StepType
