...
def train_command(
    ...
    示例:
        ...
        customer_agent train --data ./my_data --output ./my_models
        customer_agent train --dry-run  # 仅验证
    """
    verbose = ctx.obj.get("verbose", False)
    debug = ctx.obj.get("debug", False)
    
    click.echo("=" * 50)
    click.echo("CustomerAgent AI - 模型训练")
    click.echo("=" * 50)
    ...
    try:
        ...
        from customer_agent.shared.config import CustomerAgentConfig
        
        # 加载配置（用于验证）
        click.echo("加载配置...")
        customer_agent_config = CustomerAgentConfig.load(str(config_path))
        click.echo("  配置加载成功")
        ...
        if dry_run:
            ...
            from customer_agent.dialogue_understanding.flow import FlowLoader
