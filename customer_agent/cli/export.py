...
def export_command(
    ...
    示例:
        customer_agent export --output ./export/model.tar.gz
        customer_agent export --format zip --output ./export/model.zip
        customer_agent export --format json --output ./export/domain.json
    """
    ...
    click.echo("=" * 50)
    click.echo("CustomerAgent AI - 导出")
    click.echo("=" * 50)
...
def _export_json(source: Path, output: Path) -> None:
    ...
    if not domain_path.exists():
        domain_path = source / "domain.yml"
    if not domain_path.exists():
        domain_path = source / "domain.yaml"

    if not domain_path.exists():
        raise FileNotFoundError(f"未找到domain文件或domain目录: {source}")
