...
class InteractiveShell:
    ...
    def _print_welcome(self) -> None:
        ...
        click.echo()
        click.echo("=" * 50)
        click.echo("CustomerAgent AI - 交互式对话")
        click.echo("=" * 50)
        click.echo()
            ...
            if tracker:
                ...
                for turn in history:
                    ...
                    if user_message:
                        ...
                        click.echo(f"  [{index}] You: {text[:100]}...")
                        index += 1
                    for bot_message in turn.get("bot_messages", []):
                        text = bot_message.get("text") or bot_message.get("data", {}).get("text", "")
                        click.echo(f"  [{index}] Bot: {text[:100]}...")
                        index += 1
                click.echo()
            else:
                click.echo("会话不存在")
...
def shell_command(
    ...
    示例:
        ...
        customer_agent shell --model ./my_project
        customer_agent shell --sender-id test_user
    """
