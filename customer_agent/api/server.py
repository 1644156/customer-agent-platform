...
from customer_agent.channels.base_channel import UserMessage
...
class MessageResponse(BaseModel):
    ...
    crag_decision: Optional[Any] = None
...
class CustomerAgentServer:
    ...
    def __init__(
        ...
        # 消息去重：记录最近处理的消息，防止 REST + WebSocket 双通道重复处理
        self._recent_messages: Dict[str, float] = {}  # key: "sender_id:message_hash" -> timestamp
    ...
    def _create_app(self) -> FastAPI:
        """创建FastAPI应用。"""
        app = FastAPI(
            title="CustomerAgent AI",
            description="任务型客服 Agent API",
            version="0.1.0",
            docs_url="/docs",
            redoc_url="/redoc",
        )
    ...
    def _register_routes(self, app: FastAPI) -> None:
        ...
        async def root():
            """根路径健康检查。"""
            from customer_agent import __version__
            return {
                "status": "ok",
                "version": __version__,
        ...
        async def chat_page():
            ...
            return self._get_template_html("chat.html")
        ...
        async def send_message(request: MessageRequest):
            ...
            try:
                # 去重：防止 REST + WebSocket 双通道重复处理
                if self._is_duplicate(request.sender, request.message):
                    logger.warning(f"[REST] 消息被去重过滤: {request.message[:80]}")
                    return []
        ...
        async def get_session(session_id: str):
            ...
            try:
                ...
                return {
                    ...
                    "latest_message": (
                        tracker.latest_message.to_dict()
            ...
            try:
                ...
                for turn in tracker.dialogue_turns:
                    ...
                    for bot_msg in turn.bot_messages:
                        data = self._json_safe(getattr(bot_msg, "data", {}) or {})
                        event = {
                            "event": "bot",
                            "text": bot_msg.text,
            ...
            try:
                while True:
                    ...
                    if msg_type == "connect":
                        ...
                        logger.warning(f"[WebSocket] 收到消息: sender={sender_id}, text={text[:80]}")

                        # 去重：防止 REST + WebSocket 双通道重复处理
                        if self._is_duplicate(sender_id, text):
                            logger.warning(f"[WebSocket] 消息被去重过滤: {text[:80]}")
                            continue
                        ...
                        try:
                            ...
                            logger.warning(f"[WebSocket] 发送响应: {len(response.messages)} 条消息")
                            await websocket.send_json({
                                "type": "bot_response",
                            ...
                            logger.error(f"[WebSocket] 处理消息异常: {e}", exc_info=True)
                            await websocket.send_json({
                                "type": "error",
                                "message": str(e),
                            })
    ...
    def _is_duplicate(self, sender_id: str, message: str) -> bool:
        """检查消息是否为重复（2秒内同一发送者的相同消息）。"""
        import time
        key = f"{sender_id}:{hash(message)}"
        now = time.time()
    ...
    def _get_inspect_html(self) -> str:
        """获取调试页面HTML。"""
        return self._get_template_html("inspect.html")

    def _get_template_html(self, template_name: str) -> str:
        """Read an API HTML template."""
        import os
    ...
    def _json_safe(self, value: Any) -> Any:
        """Return a JSON-serializable copy of tracker payload data."""
