...
from customer_agent.agent.message_processor import (
    MessageProcessor,
    ProcessorConfig,
    MessageResponse,
)
from customer_agent.agent.actions import register_action, Action
from customer_agent.agent.graph import (
...
from customer_agent.core.stores import create_tracker_store, TrackerStore
from customer_agent.dialogue_understanding.flow import FlowsList, FlowLoader
from customer_agent.dialogue_understanding.generator import LLMCommandGenerator
from customer_agent.dialogue_understanding.processor import CommandProcessor
from customer_agent.policies import PolicyEnsemble, FlowPolicy, EnterpriseSearchPolicy
from customer_agent.shared.yaml_loader import read_yaml_file
from customer_agent.shared.config import CustomerAgentConfig, LLMConfig
...
class Agent:
    ...
    def load(
        ...
        if project_path.is_file() and project_path.name.endswith(".tar.gz"):
            ...
            # 解压到临时目录
            temp_dir = tempfile.mkdtemp(prefix="customer_agent_model_")
        ...
        if config_path.exists():
            ...
            if config_data:
                ...
                if "retrieval" in config_data:
                    from customer_agent.shared.config import RetrievalConfig
        ...
        if llm_config:
            from customer_agent.dialogue_understanding.generator import (
                LLMCommandGenerator,
                LLMGeneratorConfig,
            )
        ...
        # 创建策略
        from customer_agent.policies import EnterpriseSearchPolicyConfig
        ...
        if retriever_class_path:
            try:
                from customer_agent.retrieval import create_retriever
        ...
        if nlg_config and nlg_config.rephrase_enabled:
            try:
                from customer_agent.nlg import ResponseRephraser, RephraserConfig, TemplateNLG
