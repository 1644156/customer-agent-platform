...
from customer_agent.dialogue_understanding.flow.flow import Flow, FlowStep, FlowsList, StepType
from customer_agent.dialogue_understanding.stack.stack_frame import FlowStackFrame
...
class FlowExecutor:
    ...
    def _get_current_step_id(
        ...
        if flow_frame:
            # 优先检查嵌套步骤（在检查 step_id 之前）
            if flow_frame.has_nested_steps():
                nested = flow_frame.get_current_nested_step()
                if nested and isinstance(nested, dict):
                    nested_id = f"__nested_step_{flow_frame.nested_step_index}"
                    if nested_id not in flow._step_index:
                        from customer_agent.dialogue_understanding.flow.flow import FlowStep
                        nested_step = FlowStep.from_dict(nested_id, nested)
                        flow._step_index[nested_id] = nested_step
                    flow_frame.step_id = nested_id
                    return nested_id
    ...
    def _execute_collect_step(
        ...
        logger.warning(
            f"[FlowExecutor] collect步骤: slot={slot_name}, "
            f"current_value={current_value!r}, currently_collecting={currently_collecting}, "
            f"need_ask={need_ask}, ask_before_filling={step.ask_before_filling}"
        )
        ...
        if need_ask:
            ...
            logger.warning(f"[FlowExecutor] collect步骤 slot已填充, resolved_next={resolved_next}")
            
            # 检查是否有嵌套的动作需要执行
            nested_action = self._get_nested_action(step.next, tracker)
            if nested_action:
                # 处理 set_slots 步骤（立即执行槽位设置）
                if "set_slots" in nested_action:
                    slots = nested_action["set_slots"]
                    if isinstance(slots, list):
                        for slot_entry in slots:
                            if isinstance(slot_entry, dict):
                                for k, v in slot_entry.items():
                                    tracker.set_slot(k, v)
                                    result.events.append({"event": "slot_set", "name": k, "value": v})
                    elif isinstance(slots, dict):
                        for k, v in slots.items():
                            tracker.set_slot(k, v)
                            result.events.append({"event": "slot_set", "name": k, "value": v})
                    # set_slots 步骤没有 action，直接跳到 next
                    ...
                    logger.warning(
                        f"Executed nested set_slots, next={result.next_step_id}"
                    )
                else:
                    ...
                    # 如果嵌套步骤是 collect，传递 slot_to_collect 信息
                    if "slot_to_collect" in nested_action:
                        result.slot_to_collect = nested_action["slot_to_collect"]
                        result.metadata["slot_to_collect"] = nested_action["slot_to_collect"]
                        if nested_action.get("fallback_action"):
                            result.metadata["fallback_action"] = nested_action["fallback_action"]
                # 存储所有嵌套步骤到 frame（包括第一个，用于后续推进）
                all_steps = nested_action.get("all_steps", [])
                if len(all_steps) > 1 and flow_frame:
                    flow_frame.set_nested_steps(all_steps)
                    logger.warning(
                        f"Stored {len(all_steps)} nested steps on frame (index=0)"
                    )
                logger.warning(
                    f"Slot {slot_name} filled, executing nested action: {result.action}, "
                    f"slot_to_collect={result.metadata.get('slot_to_collect')}"
                )
            else:
                result.next_step_id = resolved_next
                logger.debug(f"Slot {slot_name} already filled, moving to: {resolved_next}")

            # 如果没有显式 next 且当前是嵌套步骤，尝试推进到下一个嵌套步骤
            if result.next_step_id is None and not result.action and flow_frame and flow_frame.has_nested_steps():
                has_more = flow_frame.advance_nested_step()
                if has_more:
                    next_nested = flow_frame.get_current_nested_step()
                    if next_nested and isinstance(next_nested, dict):
                        nested_id = f"__nested_step_{flow_frame.nested_step_index}"
                        if nested_id not in flow._step_index:
                            nested_step = FlowStep.from_dict(nested_id, next_nested)
                            flow._step_index[nested_id] = nested_step
                        result.next_step_id = nested_id
                        # 如果下一步是 collect，设置 slot_to_collect 和 action
                        if "collect" in next_nested:
                            slot = next_nested["collect"]
                            result.action = next_nested.get("action") or f"utter_ask_{slot}"
                            result.slot_to_collect = slot
                            result.metadata["slot_to_collect"] = slot
                            result.metadata["fallback_action"] = f"action_ask_{slot}"
                        elif "action" in next_nested:
                            result.action = next_nested["action"]
                        logger.warning(
                            f"Nested step completed, advancing to next: {nested_id}"
                        )
                else:
                    # 没有更多嵌套步骤，清除上下文，使用外层步骤的 next
                    flow_frame.clear_nested_steps()
                    # 重新解析外层步骤的 next（step 是嵌套步骤，需要用外层步骤的 next）
                    # 外层步骤的 step_id 在 clear_nested_steps 后会恢复
                    # 但此时 step 仍然是嵌套步骤，所以我们需要找到外层步骤
                    outer_step_id = flow_frame.step_id
                    if outer_step_id in flow._step_index:
                        outer_step = flow._step_index[outer_step_id]
                        resolved_outer_next = self._resolve_next_step(outer_step.next, tracker)
                        result.next_step_id = resolved_outer_next
                        logger.warning(
                            f"All nested steps done, using outer step next: {resolved_outer_next}"
                        )

        # 检查是否流程结束
        if not result.action and not result.slot_to_collect:
            next_val = result.next_step_id
    ...
    def _evaluate_condition(
        ...
        if "==" in condition:
            ...
            if len(parts) == 2:
                ...
                logger.warning(f"[evaluate] {slot_name}: actual={actual_value!r} (str={str(actual_value)!r}), expected={expected_value!r}, result={result}")
                return result
    ...
    def _get_nested_action(
        ...
        Returns:
            嵌套步骤中的动作信息，包含 action、next 和 remaining_steps
        """
                        ...
                        if isinstance(first_step, dict):
                            ...
                            if "action" in first_step:
                                return {
                                    ...
                                    "all_steps": then_value,
                                }
                            # 支持 collect 步骤
                            if "collect" in first_step:
                                slot_name = first_step["collect"]
                                action = first_step.get("action") or f"utter_ask_{slot_name}"
                                return {
                                    "action": action,
                                    "next": first_step.get("next"),
                                    "slot_to_collect": slot_name,
                                    "fallback_action": f"action_ask_{slot_name}",
                                    "all_steps": then_value,
                                }
                            # 支持 set_slots 步骤
                            if "set_slots" in first_step or "set_slot" in first_step:
                                return {
                                    "action": None,
                                    "next": first_step.get("next"),
                                    "set_slots": first_step.get("set_slots") or first_step.get("set_slot"),
                                    "all_steps": then_value,
                                }
                    return None
    ...
    def advance_flow(
        ...
        if flow_frame:
            ...
            # 如果下一步不是嵌套步骤，清除嵌套上下文
            if not next_step_id.startswith("__nested_step_"):
                flow_frame.clear_nested_steps()
