...
if TYPE_CHECKING:
    from customer_agent.core.tracker import DialogueStateTracker, UserMessage
    from customer_agent.core.domain import Domain
    from customer_agent.dialogue_understanding.flow import Flow, FlowsList
...
class PromptBuilder:
    ...
    def _build_template_context(
        ...
        context["current_slot_allowed_values"] = self._get_current_slot_allowed_values(
            tracker, domain
        )
    ...
    def _prepare_slot_info(
        ...
        if domain and domain.slots:
            for name, slot in domain.slots.items():
                ...
                # 获取允许的值（如果是分类槽位）
                allowed_values = getattr(slot, "allowed_values", None) or getattr(slot, "values", None)
                if allowed_values:
    ...
    def _get_current_slot_allowed_values(
        self,
        tracker: "DialogueStateTracker",
        domain: Optional["Domain"] = None,
    ) -> Optional[List[Any]]:
        """获取当前槽位的允许值列表（categorical 槽位）。"""
        current_slot = self._get_current_slot(tracker)
        if not current_slot or not domain or not domain.slots:
            return None
        slot = domain.slots.get(current_slot)
        if not slot:
            return None
        return getattr(slot, "values", None) or getattr(slot, "allowed_values", None)
