...
class FlowStackFrame(StackFrame):
    ...
    nested_steps: List[Dict[str, Any]] = field(default_factory=list)
    nested_step_index: int = 0
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "FlowStackFrame":
        ...
        return FlowStackFrame(
            ...
            nested_steps=data.get("nested_steps", []),
            nested_step_index=data.get("nested_step_index", 0),
        )
    ...
    def set_nested_steps(self, steps: List[Dict[str, Any]]) -> None:
        """设置嵌套步骤列表，索引设为0（从第一个步骤开始）。"""
        self.nested_steps = steps
        self.nested_step_index = 0

    def get_current_nested_step(self) -> Optional[Dict[str, Any]]:
        """返回当前嵌套步骤。"""
        if self.nested_steps and 0 <= self.nested_step_index < len(self.nested_steps):
            return self.nested_steps[self.nested_step_index]
        return None

    def advance_nested_step(self) -> bool:
        """推进到下一个嵌套步骤，返回是否还有更多步骤。"""
        self.nested_step_index += 1
        return self.nested_step_index < len(self.nested_steps)

    def has_nested_steps(self) -> bool:
        """检查是否有未完成的嵌套步骤。"""
        return bool(self.nested_steps) and self.nested_step_index < len(self.nested_steps)

    def clear_nested_steps(self) -> None:
        """清空嵌套步骤上下文。"""
        self.nested_steps = []
        self.nested_step_index = 0
