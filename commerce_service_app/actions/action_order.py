...
class ActionGetOrderDetail(Action):
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        if not order_id or order_id is False or order_id == "false":
            result.add_response("好的，已退出查询。")
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        if set_receive_info:
            ...
            try:
                with SessionLocal() as session:
                    # 将 detached instance 关联到当前 session
                    receive_info = session.merge(receive_info)
                    ...
                    # 更新订单的收货信息
                    new_receive_id = receive_info.receive_id
                    order_info.receive_id = new_receive_id
                    session.commit()

                result.add_response("订单收货信息已修改")
                logger.info(f"订单 {order_id} 收货信息更新为: {new_receive_id}")
